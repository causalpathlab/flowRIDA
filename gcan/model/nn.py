...
class ETMDecoder(nn.Module):
    def __init__(self, latent_dims, out_dims, jitter=0.1):
...
def train(etm, data, epochs, l_rate):
    ...
    total_batches = len(data)  # This should now work as expected
    ...
    for epoch in range(epochs):
        ...
        batch_progress = tqdm(
            data, desc=f"Epoch {epoch+1}/{epochs}", unit="batch", total=len(data)
        )
        ...
        if epoch % 10 == 0:
            logger.info(
                "====> Epoch: {} Average loss: {:.4f}".format(epoch, loss / len(data))
            )
